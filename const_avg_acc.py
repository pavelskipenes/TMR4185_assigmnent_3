...
def const_avg_acc(m, k, c, P, h, u0, udot0):
    # setter opp oppsett for om hele arrayet skal tas i en engang
     ...
     for i in range(1, len(P) - 1):#vil ikke indeksere utenfor rammer med i + 1
          ...
          u_doubledot_iplus1 = (2 * h * (P[i + 1] - k * u_iplus1 - c * u_i - c * u_doubledot_i / (2 * h))) / (2 * h * m + c) #kan være u_doubledot_i bør være u_dot_i
               
          u_dot_iplus1 = u_dot_i + 0.5 * (u_doubledot_i + u_doubledot_iplus1) * h
